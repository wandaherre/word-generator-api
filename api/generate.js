...
// Word-Boxes: nur aus *strukturierter* Quelle; kein „DOM-Fallback“, kein Ratespiel
function normalizeWordBoxes(payload) {
  Object.keys(payload).forEach(k => {
    if (/_word_box_content$/i.test(k)) {
      const base = k.replace(/_word_box_content$/i, "");
      const arr = toArrayFlexible(payload[k]); // robust zu Array normalisieren

      // Heuristik: Nur setzen, wenn das wirklich nach „Wortliste“ aussieht
      // (max. 24 Einträge, jeder < 50 Zeichen)
      const looksLikeWordBox = arr.length > 0 && arr.length <= 24 && arr.every(x => x.length > 0 && x.length < 50);

      if (looksLikeWordBox) {
        // line nur bauen, wenn nicht schon vorhanden (Frontend priorisieren)
        if (!payload[`${base}_word_box_content_line`]) {
          payload[`${base}_word_box_content_line`] = arr.join("   |   ");
        }
        // und *_word_box_content als String halten
        payload[k] = arr.join(" | ");
      } else {
        // ansonsten *gar nichts* setzen (kein falscher/überflüssiger Box-Content)
        delete payload[k];
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    // 3) Word-Boxes NUR aus strukturierter Quelle (kein DOM/kein Fallback)
